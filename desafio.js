function calcularRanrinkMedalhas () {
  let pais = [];
  let numeroDeMedalhas = [];
  let totalMedalhas = 0;
  while (true) {
    let pais = prompt ("Nome do país" (ou 'sair' para terminar):);
    if (pais = "sair") {
      break;
    }

    let numeroDeMedalhas = number (prompt("Número de medalhas"))
    if (!isNaN(numeroDeMedalhas))
      alert("Por favor, insira um numero válido.");

    for (let i = 1; i <numeroDeMedalhas.length; i++) {
      if (numeroDeMedalhas[i] > maiorNumero) {
        maiorNumero = medalhas[i];
  
      }
      if (numeroDeMedalhas[i] < menorNumero) {
        menorNumero = medalhas[i];
        paisMenorNumero = pais[i];
      }

    alert(`Ranking de Medalhas:
      (/* não consegui fazer a parte do ranking)
